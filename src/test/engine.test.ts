...
//  (the engine's only deployment model) with frictionCoeff = 0 for clean
//  conservation. Body dynamics are pure free-float — only internal hinge
//  reaction torques act.
// ─────────────────────────────────────────────────────────────────────────────
...
// NOTE: the legacy engine-level `computeReportData` (a duplicate, kinematic-configured
// report path unused by the UI) was removed with the kinematic deployment mode; its
// test block was removed with it. The scientific report path is reportData.ts,
// covered by reportData.test.ts and deploymentMode.test.ts.
